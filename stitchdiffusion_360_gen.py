    ...
    def __call__(
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            views_t = self.get_views(height, width)
            ...
            for i, t in enumerate(timesteps):
                ...
                for ii_md in range(2):
                    ...
                    latent_model_input = torch.cat([latent_view_t] * 2) if self.do_classifier_free_guidance else latents_view_t
                    ...
                    latent_view_denoised = self.scheduler.step(noise_pred, t, latent_view_t, **extra_step_kwargs, return_dict=False)[0]
                ...
                for h_start, h_end, w_start, w_end in views_t:
                    ...
                    # expand the latents if we are doing classifier free guidance
                    latent_model_input = torch.cat([latent_view_t] * 2) if self.do_classifier_free_guidance else latent_view_t
                    latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
...
if __name__ == "__main__":
    ...
    pipe.load_lora_weights("YOUR_PATH/lora_modified.safetensors")
    _ = pipe.to("cuda")

    prompt = "360-degree panoramic image, Hogwarts campus, hyper realistic"
    negative_prompt = "lowres, bad anatomy, bad hands, text, error, missing fingers, extra digit, fewer digits, cropped, worst quality, low quality, normal quality, jpeg artifacts, signature, watermark, username, blurry"

    set_seed(seed)
    generator = torch.Generator(device='cuda').manual_seed(seed)

    image = pipe(prompt=prompt, generator=generator, num_inference_steps=50, guidance_scale=7.5, height=512, width=2048,
                 negative_prompt=negative_prompt).images[0]
    name_360_img = f"seed_{seed}-prompt_{prompt}.png"
    image.save(name_360_img)